...
void run_child(int msqid, long time) {
...
void run_parent(key_t key, long time) {
    struct my_msgbuf buf;
    int msqid;
    long count = 0 ;
    ...
    do { /* parent process se lien tuc doc message tu queue*/
        msgrcv(msqid, &buf, sizeof(buf.mtext), count, 0); 
