...
const char resultFile[] = "time_file.txt";

void readStringToFIle(const char* fileName, char* buf) {
    int fd;
...
void writeStringToFIle (const char* fileName, struct flock* fl) {
...
int main(int argc, char *argv[]) {
    ...
    struct timespec wStart, wStop, rStart, rStop;
    ...
    if (writeProcess > 0) {
        // khoa file tranh truong hop doc ghi lan lon
